...
/* 点击减号 */
/**
 *  传入参数{e:点击事件，that:作用域，data:数组数据}
 */
var bindMinus = (e, that, data, dataName) => {
  ...
  var maxStatus = (num == quality) ? 'disabled' : 'normal';
  var numId = dataName + '[' + i + '].num';
  var maxStatusStr = dataName + '[' + i + '].maxStatus';
  var minStatusStr = dataName + '[' + i + '].minusStatus';
  // 将数值与状态写回  
  that.setData({
    [numId]: num,
    [minStatusStr]: minusStatus,
    [maxStatusStr]: maxStatus
  });
}


/* 点击加号 */
var bindPlus = (e, that, data, dataName) => {
  var i = e.currentTarget.dataset.index;
  var num = data[i].num;
  var quality = data[i].quality;
  // 不作过多考虑自增1  
...
module.exports.bindMinus = bindMinus;
module.exports.bindPlus = bindPlus;
